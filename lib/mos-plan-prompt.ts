...
## BẢNG TỪNG BÀI (items) — BẮT BUỘC
Với MỖI dòng bài trong plan, trả về 1 phần tử items[]: no (số thứ tự), topic (tên bài/angle, ≤80 ký tự), product (model/nhóm, "" nếu không có), journey ("Awareness|Consideration|Conversion|Retention" — quy đổi TOFU→Awareness, MOFU→Consideration, BOFU→Conversion, Referral→Retention; dùng BẢN CHẤT bài, không dùng lời khai), channel (kênh dự kiến, "" nếu plan không ghi), gate ("PASS" nếu dòng này không có vấn đề gì; "WARN" nếu có vấn đề nhưng vẫn viết được; "FAIL" nếu dòng này dính lỗi chặn: nhầm model, claim y tế, sai kênh nặng, lộ giá sỉ), score (0-100 sức khỏe của riêng dòng đó), issue (1 câu ≤120 ký tự nêu vấn đề lớn nhất của dòng; "" nếu không có).
Nếu plan quá dài, vẫn liệt kê ĐỦ mọi dòng — chỉ rút gọn chữ, KHÔNG bỏ dòng. Không bịa dòng không có trong plan.

## VẤN ĐỀ CẤP PLAN (plan_issues) — tối đa 5
Chỉ nêu vấn đề CHIẾN LƯỢC của cả plan, không phải lỗi của 1 dòng: phân bổ phễu mất cân đối · thiếu bài Conversion · concept/angle lặp · CTA lặp hoặc yếu · USP không đa dạng · một model xuất hiện quá nhiều · thiếu proof · claim chưa xác minh. MỖI vấn đề tối đa 1-2 CÂU, có con số nếu đếm được. Cấm viết đoạn dài.

## OUTPUT — TRẢ VỀ DUY NHẤT JSON:
{"plan_title":"tên/mô tả ngắn plan","total_items":số dòng bài đếm được,"items":[{"no":int,"topic":str,"product":str,"journey":str,"channel":str,"gate":"PASS|WARN|FAIL","score":int,"issue":str}],"plan_issues":[str tối đa 5],"score":int 0-100,"decision":"PASS|MINOR_FIX|MAJOR_FIX|REWRITE","dimensions":{"journey":{"score":int 0-20,"issues":[str],"phu_song":"vd: DN304 thiếu TOFU; Vu Lan chỉ có TOFU không có bài chốt","ty_le_phieu":"vd: TOFU 45% / MOFU 20% / BOFU 35% / Retention 0%","sai_kenh":[str]},"product":{"score":int,"issues":[str]},"legal":{"score":int,"issues":[str]},"attractiveness":{"score":int,"issues":[str],"best_hook":str,"weakest_hook":str},"structure":{"score":int,"issues":[str]}},"blocking_issues":["lỗi chặn phải sửa trước khi cho viết — ghi rõ Ở DÒNG/NHÓM NÀO"],"recommendations":["đề xuất cụ thể, hành động được ngay"],"summary":"3-4 câu tổng kết cho Marketing Director"}
